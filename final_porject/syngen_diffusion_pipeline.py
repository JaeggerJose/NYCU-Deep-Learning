...
class AttentionStore:
    ...
    def aggregate_attention(self, from_where=("up", "down", "mid")):
        # 簡化實現，返回適當大小的張量
        return torch.zeros((1, self.attn_res[0] * self.attn_res[1], 77))
...
class AttendExciteAttnProcessor:
    def __init__(self, attnstore, place_in_unet):
        ...
        self.original_processor = AttnProcessor2_0()
        
    def __call__(self, attn, hidden_states, encoder_hidden_states=None, attention_mask=None, **kwargs):
        # 直接使用原始的注意力處理器，避免復雜的批次處理邏輯
...
class SynGenDiffusionPipeline(StableDiffusionPipeline):
    def __init__(self,
        ...
        if safety_checker is False:
            safety_checker = None
        if feature_extractor is False:
            feature_extractor = None
        if requires_safety_checker is False:
