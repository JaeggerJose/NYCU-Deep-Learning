...
def _symmetric_kl(attention_map1, attention_map2):
    ...
    if len(attention_map2.shape) > 1:
        ...
        # --- 修正開始 ---
    # 轉成 float32 避免 float16 精度問題
    attention_map1 = attention_map1.float()
    attention_map2 = attention_map2.float()

    # clamp 避免 0 值
    attention_map1 = attention_map1.clamp(min=1e-8)
    attention_map2 = attention_map2.clamp(min=1e-8)
