#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# PPO Inference Script for Consecutive Seeds Testing
...
class Actor(nn.Module):
    def __init__(self, in_dim: int, out_dim: int, log_std_min: int = -20, log_std_max: int = 2):
...
class PPOInference:
    ...
    def select_action(self, state: np.ndarray, deterministic: bool = True) -> np.ndarray:
        ...
        with torch.no_grad():
            ...
            raw_action, dist = self.actor(state_t)
    ...
    def find_consecutive_good_seeds(self, window_size: int = 20, 
        ...
        if found_windows:
            ...
            if len(all_results) >= window_size:
                best_avg = max([np.mean([s for _, s in all_results[i:i+window_size]]) 
                              for i in range(len(all_results)-window_size+1)])
                print(f"  Best average found: {best_avg:.2f}")
            return None
...
def main():
    parser = argparse.ArgumentParser(description="PPO Pendulum Inference for Consecutive Seeds")
    parser.add_argument("--checkpoint", type=str, required=True,
                       help="Path to PPO checkpoint file")
    parser.add_argument("--mode", type=str, choices=["consecutive", "quick", "find", "both"], default="consecutive",
                       help="Mode: 'consecutive' for finding continuous good seeds, 'quick' for quick consecutive test, 'find' for finding best seeds, 'both' for both")
    parser.add_argument("--window-size", type=int, default=20,
                       help="Number of consecutive seeds to test")
    ...
    agent = PPOInference(args.checkpoint, device=args.device)
    ...
    if args.mode == "quick":
        result = agent.quick_test_consecutive(args.start_seed, args.window_size)
    ...
    # Find best individual seeds mode
