...
def evaluate(args):
    ...
    model = DQN(4, num_actions).to(device)  # CartPole obs dim = 4
    model.load_state_dict(torch.load(args.model_path, map_location=device))
    model.eval()

    os.makedirs(args.output_dir, exist_ok=True)

    # 從 args.seed 開始，連續 20 個 seed
    for ep in range(args.episodes):
        ...
        out_path = os.path.join(args.output_dir, f"eval_seed{current_seed}.mp4")
        with imageio.get_writer(out_path, fps=30) as video:
            for f in frames:
                video.append_data(f)
        print(f"[Seed {current_seed}] Total reward: {total_reward} → saved to {out_path}")
