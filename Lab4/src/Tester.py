...
class Dataset_Dance(torchData):
    def __init__(self, root, transform, mode='test', video_len=7, partial=1.0):
        ...
        # Count actual test directories instead of relying on Demo_Test
        test_img_dirs = glob(os.path.join(root, 'test/test_img/*'))
...
class Test_model(VAE_Model):
    ...
    def forward(self, img, label):
        ...
        fused_feature = self.Decoder_Fusion(img_feature, label_feature, z)
        generated_frame = self.Generator(fused_feature)
        
        return generated_frame, mu, logvar     
    ...
    def eval(self):
        ...
        # Check if pred_seq_list is empty
        if len(pred_seq_list) == 0:
            print("Warning: No test data found. Please check that test data exists in the specified directories.")
            print("Expected test data structure:")
            print(f"  - {self.args.DR}/test/test_img/{{i}}/* (image files)")
            print(f"  - {self.args.DR}/test/test_label/{{i}}/* (label files)")
            return
        
        # submission.csv is the file you should submit to kaggle
        # Clamp values to [0,1] range before converting to prevent overflow
        pred_tensor = torch.cat(pred_seq_list)
        pred_clamped = torch.clamp(pred_tensor, 0.0, 1.0)
        
        # Handle NaN and infinity values before casting
        pred_numpy = pred_clamped.numpy()
        pred_numpy = np.nan_to_num(pred_numpy, nan=0.0, posinf=1.0, neginf=0.0)
        pred_to_int = (np.rint(pred_numpy*255)).astype(int)
    ...
    def val_one_step(self, img, label, idx=0):
        ...
        for i in range(0, 629):
            ...
            # Generate new frame
            generated_frame = self.Generator(fused_feature)
            #generated_frame_clamped = torch.clamp(generated_frame, 0.0, 1.0)
            # Store results
            decoded_frame_list.append(generated_frame.cpu())
            #decoded_frame_list.append(generated_frame_clamped.cpu())
